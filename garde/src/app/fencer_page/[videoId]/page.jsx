...
import SidebarMenu from "../../../components/fencer_page/Hamburger";
...
export default function Fencer_Page2() {
		...
		const checkQueries = async () => {
			try {
				...
				const res = await axios.post(
					"/api/get-queries",
					{ fingerprint: fp, query: null, loggedIn: !!fencerId },
					{
				...
				if (res.data.chats) {
					setChatCount(res.data.chats);
				} else {
					setChatCount(0);
				}
				if (res.data.videos) {
					setVideoCount(res.data.videos);
				} else {
					setVideoCount(0);
				}
			} catch (error) {
	...
	const handleHeightSave = useCallback(async (heightInMeters) => {
		if (!isRecording) {
			const videos = await addQuery();
			if (videos) {
